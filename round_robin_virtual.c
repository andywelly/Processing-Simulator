...
void round_robin_virtual(queue_t *queue, int quantum, int num_processes) {
    ...
    while (num_finished_processes < num_processes) {
        ...
        if (current_process == NULL && !is_empty(ready_queue)) {
            ...
            if (current_process -> num_allocated_pages < MIN_PAGES_REQUIRED) {
                allocate_pages_virtual(page_array, current_process, num_required_pages, clock);
            }
        ...
        // A process was running, and other processes waiting, next process runs once
            ...
            if (next_process -> arrival_time <= clock) {
                ...
                } // else already allocated, it just runs
